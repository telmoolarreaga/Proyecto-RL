...
def create_fixed_obstacles(field_length, base_height=0.4, cube_size=0.5):
    """
    Crea obstáculos con posiciones fijas simulando una formación defensiva
    dentro del campo.
    """

    # Formación tipo fútbol 4-4-2 (simétrica)
    # Coordenadas (x, y) centradas en el campo
    # Puedes mover o añadir más fácilmente
    ...
    for (x, y) in formation_positions:
        ...
        # Altura correcta sobre el césped
        z = base_height + cube_size / 2
        pos = [x, y, z]
        ...
        p.changeDynamics(obstacle_id, -1,
                         lateralFriction=1.0,
                         rollingFriction=0.1,
...
def create_stadium(side_length, num_obstacles=10):
    """
    Creates a soccer stadium with goals, walls, turf and obstacles.
    """
    ...
    pybullet_wall_ids = _create_perimeter(
        side_length,
        thickness=0.1,
        height=0.4,
    ...
    # Goals
    pybullet_goal_right_id = _create_goal(
        [side_length/2 - 0.10, 0, base_height],
        p.getQuaternionFromEuler([0, 0, 0])
    )
    pybullet_goal_left_id = _create_goal(
        [-side_length/2 + 0.10, 0, base_height],
    ...
    if num_obstacles > 0:
        obstacle_ids = create_obstacles(
            ...
            field_length=side_length,
            base_height=base_height,
            min_distance_from_goals=2.5,
            cube_size=0.5
        )
