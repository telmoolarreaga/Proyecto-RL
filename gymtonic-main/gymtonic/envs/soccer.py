...
from .soccer_stadium import create_stadium, create_player, create_ball, create_fixed_obstacles
...
class SoccerSingleEnv(Env):
    ...
    def __init__(self, max_speed=1.0, perimeter_side=10.0, goal_target='left', render_mode=None, record_video_file=None):
        ...
        self.previous_distance_to_ball = None
        self.previous_ball_goal_distance = None
    ...
    def step(self, action: ActType):
        ...
        goal_id = self.pybullet_goal_right_id if self.goal_direction == 'right' else self.pybullet_goal_left_id
        goal_pos, _ = p.getBasePositionAndOrientation(goal_id)

        player_xy = np.array(player_pos[:2])
        ball_xy = np.array(ball_pos[:2])
        goal_xy = np.array(goal_pos[:2])

        # Goal or termination conditions
        goal = self.is_goal()
        ...
        if self.is_ball_out_of_bounds():
            reward -= 5.0
            terminated = True
            obs = self.get_observation()
            return obs, reward, terminated, truncated, info

        # Progess towards ball
        dist_player_ball = np.linalg.norm(player_xy - ball_xy)
        if self.previous_distance_to_ball is not None:
            delta = self.previous_distance_to_ball - dist_player_ball
            reward += 0.2 * max(delta, 0.0)  
        self.previous_distance_to_ball = dist_player_ball
        ...
        if self.player_touched_ball:
            ...
            dist_ball_goal = np.linalg.norm(ball_xy - goal_xy)
            if self.previous_ball_goal_distance is not None:
                delta = self.previous_ball_goal_distance - dist_ball_goal
                reward += 2.0 * max(delta, 0.0)
            self.previous_ball_goal_distance = dist_ball_goal

        # Penalization for moving without control
        _, angular_vel = p.getBaseVelocity(self.pybullet_player_id)
        reward -= 0.05 * abs(angular_vel[2])

        # Penalization for hitting obstacles
    ...
    def is_player_touching_ball(self) -> bool:
        contacts = p.getContactPoints(self.pybullet_player_id, self.pybullet_ball_id)
        return len(contacts) > 0

    def kick_ball(self):
        orientation = self.get_orientation(self.pybullet_player_id)
        force = [math.cos(orientation) * 20, math.sin(orientation) * 20, 5]  # X, Y, Z
        position, _ = p.getBasePositionAndOrientation(self.pybullet_ball_id)
        p.applyExternalForce(self.pybullet_ball_id, -1, force, position, p.WORLD_FRAME)
